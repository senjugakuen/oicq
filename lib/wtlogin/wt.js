...
class WtLogin {
    ...
    set sex(val) { this.c.nickname = val; }
    set age(val) { this.c.nickname = val; }
    ...
    _decodeLoginResponse(blob, token = false) {
        blob = tea.decrypt(blob.slice(16, blob.length - 1), this.ecdh.share_key);
    ...
    _buildLoginPacket(cmd, body, type) {
        ...
        let sso = new Writer().writeU32(this.seq_id)
            ...
            .writeWithLength(cmd)
            .writeWithLength(this.c.session_id)
    ...
    async exchangeEMP() {
        if (!this.c.isOnline() || timestamp() - this.sig.emp_time < 14400)
            return;
